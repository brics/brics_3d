/**
 * @file
 * icp_alpha_shapes_stream_test.cpp
 * 
 * @brief Test file to demonstrate ICP from 6dslam library with alpha shapes by CGAL 3.4 and steamed depth images
 *
...
int main(int argc, char **argv) {
    ...
    if(argc != 2) {
        ...
        cerr << "Try for example: " << argv[0] << " ../test/test_data/depth_images/zcam_stream.avi" << endl;
        return -1;
    }
    ...
    /* parameters */
    int maxNumberOfFrames = 100; //shortcut to stop video sequence earlier
    double octreeSize = 3.0;
    bool saveSingelClouds = true;
	...
	DepthImageToPointCloudTransformation *img2cloudTransformer = new DepthImageToPointCloudTransformation();
  	...
  	while (true) {
  		...
  		img2cloudTransformer->transformDepthImageToPointCloud(depthImage,
  				tmpPointCloud, 10);
  		cout << "Size of point cloud" << imgCount <<": " << tmpPointCloud->getSize() << endl;
  		pointClouds->push_back(tmpPointCloud);
  		if (saveSingelClouds) {
//			stringstream tmpString;
//			tmpString << imgCount;
//  		string singleCloudName = "singlePoints";
//  		singleCloudName.append(tmpString.str());
//  		singleCloudName.append(".ply");
  			stringstream singleCloudName;
  			singleCloudName << "singlePoints" << imgCount << ".ply";
  			tmpPointCloud->storeToPlyFile(singleCloudName.str());
  		}
	...
	/* set ICP parameters */
	double red = octreeSize;//5; //paremetrize octree (-1.0 is none)
...
//	     redptsout.clear();
	   ...
	   for (; i > 0; i--) {
			//is >> p;
		   Point tmpPoint;
		   x = (*registeredPoints->getPointCloud())[i].getX();
		   y = (*registeredPoints->getPointCloud())[i].getY();
		   z = (*registeredPoints->getPointCloud())[i].getZ();
		   CGAL_Point p(x, y, z);
		   //p.PointC3(x,y,z);
